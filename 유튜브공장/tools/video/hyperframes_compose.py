...
class HyperFramesCompose(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "workers": {
                ...
                "default": 4,
                "description": (
                    "HyperFrames capture workers. Four is the safe project "
                    "default for the current Apple Silicon/V8 heap; raise it "
                    "only after a measured render test."
                ),
            },
            "strict": {
                "type": "boolean",
    ...
    def _render(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        workers = int(inputs.get("workers", 4))
        if not 1 <= workers <= 8:
            return ToolResult(
                success=False,
                error="workers must be between 1 and 8",
                data={"steps": steps},
            )
        args = [
            "render",
